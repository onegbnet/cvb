...
export function createInlineRows({ fields, items, onChange }) {
  ...
  const root = h('div', {
    ...
    style: { '--inl-cols': `repeat(${fields.length}, minmax(0, 1fr))` },
  });
  ...
  root.append(
    h(
      'div',
      { class: 'inl-head' },
      fields.map((f) => h('span', { class: 'inl-th' }, tr(f.labelKey))),
  ...
  function buildRow(record, isDraft) {
    ...
    const commit = () => {
      ...
      if (draft) {
        ...
        for (const el of row.querySelectorAll('input, textarea')) el.placeholder = '';
        slot = { ...buf };
        committed = { ...buf };
        data.push(slot);
    ...
    for (const field of fields) {
      // **按字段类型分派**,别一律 input[type=text]:`lines`/`tags` 是多值,
