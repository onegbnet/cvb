...
// ---- 多语种事实(2026-08-22)----
// factsLang = 当前打开的**事实语言**(文档身份,由 ?flang= 定,缺省默认语种)。
// 所有落库(flushSave / 快照)都按它走。**语种平权**(2026-08-23):没有哪一份是
// 「真相源」,指针只剩「默认语种」的管线语义 —— 不带 ?flang= 的读取(含生成侧)用哪份。
//
...
// 补上那门语言的界面包,耦合自动成立。
let factsLang = null;
// source 为 null = 空库,默认语种尚未确立(由第一笔事实确立;见 worker)
...
async function main() {
  ...
  // 事实语言:?flang= 指定,没列出的/没带的回落默认语种(worker 侧同一约定)
  try {
    langsInfo = await listFactsLangs();
  } catch { /* 语言清单取不到就按单语世界画,编辑照常 */ }
  // 缺省文档跟随界面语言(全站一个语言概念):界面语言对应的语种有事实就开它,
  // 没有才回默认语种 —— 从首页点进来的就是刚才看的那份。?flang= 显式指定仍最优先。
  //(/api/resume 不带参数 = 默认语种的生成侧契约不受此影响,这只是编辑器选开哪份)
