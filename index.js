...
`,cookieSecure:"auto",openWhenUnset:!1}),AI_ROUTES={"/api/ai/improve":import_ai.handleAIImproveRequest},json=(statusCode,payload,headers={})=>new Response(JSON.stringify(payload),{status:statusCode,headers:{"Content-Type":"application/json; charset=utf-8",...headers}}),unauthorized=()=>json(401,{error:{code:"UNAUTHORIZED",message:"Unlock required"}}),schemaReady=null,ensureSchema=env=>schemaReady??=env.DB.prepare(`CREATE TABLE IF NOT EXISTS resumes (
       id TEXT PRIMARY KEY,
       config TEXT NOT NULL,
       created_at INTEGER NOT NULL,
       updated_at INTEGER NOT NULL
     )`).run(),cdnHost=(env,request)=>env.CCS_CDN_BASE||selectJsdelivrCdnHost(request),CCS_PIN="098c5d6f129573a2def37d953be15caa470fb420",SUPPORTED_LANGS=["zh-cn","en"],resolveLang=request=>{let m=(request.headers.get("Cookie")||"").match(/(?:^|;\s*)lang=([^;]+)/);return m&&SUPPORTED_LANGS.includes(m[1])?m[1]:detectLangFromAcceptLanguage(request.headers.get("Accept-Language")||"",SUPPORTED_LANGS)},APP_LICENSE_URL="https://github.com/onegbnet/cvb/blob/main/LICENSE",appAssetUrl=(relativePath,request)=>`https://${selectJsdelivrCdnHost(request)}/gh/onegbnet/cvb@94d3f57342b4114083bbc44c393b1ce577669c23/cvb/${relativePath}`,serveHtmlAsset=async(request,env,lang)=>{let res=await env.ASSETS.fetch(request),contentType=res.headers.get("Content-Type")||"";if(!res.ok||!contentType.includes("text/html"))return res;let texEngine=renderTexEngineScript(buildTexEngineConfig({request,env,ccsSha:CCS_PIN})),html=(await res.text()).replaceAll("{{LANG}}",lang).replaceAll("{{CCS_CDN}}",cdnHost(env,request)).replaceAll("{{CCS_PIN}}",CCS_PIN).replaceAll(TEX_ENGINE_PLACEHOLDER,texEngine).replace(/(\b(?:src|href)=['"])(app|static)\//g,(_,prefix,directory)=>`${prefix}${appAssetUrl(`${directory}/`,request)}`).replaceAll('href="/LICENSE"',`href="${APP_LICENSE_URL}"`),headers=new Headers(res.headers);return headers.delete("Content-Length"),new Response(html,{status:res.status,headers})},worker_default={async fetch(request,env){let url=new URL(request.url),{pathname}=url,method=request.method;if(method==="GET"&&pathname==="/health")return json(200,{ok:!0});if(pathname==="/unlock")return method==="POST"?lock.handleUnlock(request,env):await lock.isAuthorized(request,env)?Response.redirect(new URL("/",url).toString(),302):lock.renderLockPage(cdnHost(env,request));if(method==="GET"&&pathname==="/api/auth")return json(200,{authed:await lock.isAuthorized(request,env)});if(method==="GET"&&pathname===TEX_WORKER_SHIM_PATH){let shim=handleTexWorkerShimRequest({request,env,ccsSha:CCS_PIN});return shim||json(404,{error:{code:"NOT_FOUND",message:"TeX engine disabled"}})}if(method==="POST"&&pathname==="/api/prefs")return handlePrefs(request);if(pathname==="/api/resume"){if(await ensureSchema(env),method==="GET"){let row=await env.DB.prepare("SELECT config, updated_at FROM resumes WHERE id = ?").bind(RESUME_ID).first();return row?json(200,{config:JSON.parse(row.config),updatedAt:row.updated_at}):json(404,{error:{code:"RESUME_NOT_FOUND",message:"No resume stored yet"}})}if(method==="PUT"){if(!await lock.isAuthorized(request,env))return unauthorized();let config;try{config=await request.json()}catch{return json(400,{error:{code:"BAD_REQUEST",message:"Body must be valid JSON"}})}if(!config||typeof config!="object"||Array.isArray(config))return json(400,{error:{code:"BAD_REQUEST",message:"Config must be an object"}});let serialized=JSON.stringify(config),size=checkConfigSize(serialized);if(!size.ok)return json(413,{error:{code:"TOO_LARGE",message:`Config is ${size.bytes} bytes, limit is ${size.limit}`,bytes:size.bytes,limit:size.limit}});let now=Date.now();return await env.DB.prepare(`INSERT INTO resumes (id, config, created_at, updated_at) VALUES (?, ?, ?, ?)
           ON CONFLICT(id) DO UPDATE SET config = excluded.config, updated_at = excluded.updated_at`).bind(RESUME_ID,serialized,now,now).run(),json(200,{ok:!0,updatedAt:now})}return json(405,{error:{code:"METHOD_NOT_ALLOWED",message:"Use GET or PUT"}})}if(method==="POST"&&pathname==="/api/avatar"){if(!await lock.isAuthorized(request,env))return unauthorized();let contentType=(request.headers.get("Content-Type")||"").split(";")[0].trim(),ext=AVATAR_TYPES[contentType];if(!ext)return json(415,{error:{code:"UNSUPPORTED_TYPE",message:"png/jpeg/webp/gif only"}});let body=await request.arrayBuffer();if(body.byteLength===0||body.byteLength>MAX_AVATAR_BYTES)return json(413,{error:{code:"TOO_LARGE",message:"Avatar must be 1B-2MB"}});let key=`avatars/${Date.now()}.${ext}`;return await env.FS.put(key,body,{httpMetadata:{contentType}}),json(200,{ok:!0,url:`/files/${key}`})}if(method==="POST"&&pathname==="/api/export"){if(!await lock.isAuthorized(request,env))return unauthorized();await ensureSchema(env);let row=await env.DB.prepare("SELECT config FROM resumes WHERE id = ?").bind(RESUME_ID).first();if(!row)return json(404,{error:{code:"RESUME_NOT_FOUND",message:"Nothing to export"}});let key=`exports/resume-${new Date().toISOString().replace(/[:.]/g,"-")}.json`;return await env.FS.put(key,row.config,{httpMetadata:{contentType:"application/json; charset=utf-8"}}),json(200,{ok:!0,url:`/files/${key}`})}if(method==="GET"&&pathname==="/api/exports"){if(!await lock.isAuthorized(request,env))return unauthorized();let list=await env.FS.list({prefix:"exports/"});return json(200,{exports:list.objects.map(o=>({key:o.key,url:`/files/${o.key}`,size:o.size,uploaded:o.uploaded}))})}if(method==="GET"&&pathname.startsWith("/files/")){let key=decodeURIComponent(pathname.slice(7));if(!(key.startsWith("avatars/")&&!key.includes(".."))&&!await lock.isAuthorized(request,env))return unauthorized();let object=await env.FS.get(key);if(!object)return json(404,{error:{code:"NOT_FOUND",message:"File not found"}});let headers=new Headers;return object.writeHttpMetadata(headers),headers.set("Cache-Control","public, max-age=31536000, immutable"),new Response(object.body,{headers})}let aiHandler=AI_ROUTES[pathname];if(aiHandler){if(method!=="POST")return json(405,{error:{code:"METHOD_NOT_ALLOWED",message:"Use POST"}});if(!await lock.isAuthorized(request,env))return unauthorized();let body;try{body=await request.json()}catch{return json(400,{error:{code:"AI_BAD_REQUEST",message:"Request body must be valid JSON"}})}let result=await aiHandler(body,{env});return json(result.statusCode,result.payload)}let PAGE_ASSETS={"/":"home.html","/edit":"index.html","/apply":"preview.html"};if(method==="GET"&&["/index.html","/preview","/preview.html"].includes(pathname))return new Response("Not Found",{status:404});if(method==="GET"&&PAGE_ASSETS[pathname]){if((pathname==="/"||pathname==="/edit")&&!await lock.isAuthorized(request,env))return lock.renderLockPage(cdnHost(env,request));let assetUrl=new URL(`/${PAGE_ASSETS[pathname]}`,url);return serveHtmlAsset(new Request(assetUrl,request),env,resolveLang(request))}return env.ASSETS.fetch(request)}};export{worker_default as default};
